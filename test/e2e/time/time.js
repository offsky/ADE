...
describe('time', function() {
    ...
    it('should dismiss a popup on ENTER', function() {
        ...
        expect(element('.ade-editme:eq(0)').text()).toBe(expectedHour+':59 '+expectedAmPM);
    ...
    it('should not allow invalid input', function() {
        element('.ade-editme:eq(0)').click();
        appElement('.ade-editme + input', function(elm) {
            elm.trigger({ type : 'keypress', keyCode: 98 }); //letter b
            elm.trigger({ type : 'keydown', keyCode: 13 });
        });
        expect(element('.ade-editme:eq(0)').text()).toBe('3:59 pm');
        expect(element('.ade-editme + input').count()).toEqual(0);
    });
    ...
    it('should allow entering value into input', function() {
        ...
        expect(element('.ade-editme:eq(0)').text()).toBe('6:00 am');
        expect(element('.ade-editme + input').count()).toEqual(0);
    });
    ...
    it('should edit/save entry with ENTER', function() {
        ...
        expect(element('.ade-editme:eq(0)').text()).toBe((expectedHour+1)+':00 '+expectedAmPM); //TODO: if you are in the right timezone, this will fail becaus we are not switching am to pm
        expect(element('.ade-editme + input').count()).toEqual(0);
    });
    ...
    it('should edit/save entry with TAB', function() {
        ...
        expect(element('.ade-editme:eq(0)').text()).toBe((expectedHour+1)+':59 '+expectedAmPM); //TODO: if you are in the right timezone, this will fail becaus we are not switching am to pm
        expect(element('.ade-editme + input').count()).toEqual(0);
    });
