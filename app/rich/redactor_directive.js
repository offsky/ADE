...
angular.module('ADE').directive('adeRedactor', ['ADE', '$compile', '$sanitize', function(ADE, $compile, $sanitize) {
	return {
		...
		link: function(scope, element, attrs) {
			...
			var saveEdit = function(exited) {
				...
				var content = $('.redactor_');
				var currentLength = content.text().length;
				...
				if ((exited != 3) && (!maxLength || (currentLength <= maxLength))) {
					...
					// if (quill)
					if (true) { //if we can't find the editor, dont overwrite the old text with nothing. Just cancel
	...
	// Rich Text Editor
			...
			var editRichText = function() {
				...
				var content = '<textarea id="content" name="content">' + modelValue + '</textarea>';
			...
			var outerBlur = function(e) {
				...
				// + id
				var outerClick = $('.redactor_box').has(e.target).length === 0;
