	...
	return {
		...
		link: function($scope, element, attrs, controller) {
			...
			element.bind('click', function() {
				...
				$compile('<input type="hidden" ui-select2="{width:\'resolve\',allowClear:true,openOnEnter:false,allowAddNewValues:true,query:query'+listid+'}" '+multi+' data-placeholder="List..." />')($scope).insertAfter(element);
			...
			return attrs.$observe('adeList', function(settings) { //settings is the contents of the ade-list="" string
				options = ADE.parseSettings(settings, {multiple:false});
				return element;
			});
		}
	};
